...
const BotTester = ({ isOpen, onClose, botConfig }) => {
  ...
  const [screenSize, setScreenSize] = useState('large'); // State for screen size
  const [reduceMotion, setReduceMotion] = useState(false); // State for reduced motion
  ...
  useEffect(() => {
    if (isOpen && messages.length === 0 && botConfig.welcomeMessage) {
      setMessages([{
        role: 'assistant',
  ...
  }, [isOpen, botConfig.welcomeMessage, messages.length]);
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      console.log('Sending request to AI service with prompt:', inputMessage);
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className={`fixed inset-0 z-50 flex items-center justify-center ${reduceMotion ? 'reduce-animations' : ''} major-feature-section`}
          style={{ 
            backgroundColor: 'var(--dark-bg-color, #121622)' // Dark background instead of blur
          }}
        >
          <div className="max-w-5xl mx-auto p-4 w-full">
            <div className="w-full rounded-2xl glassmorphism p-6 border border-white/10 overflow-hidden">
              {/* Header with Close Button */}
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-white">
                  Test Your Chatbot in a Website Environment
                </h2>
                <motion.button
                  onClick={handleClose}
                  className="text-white hover:text-indigo-400 transition-colors duration-200 bg-white/10 h-10 w-10 rounded-full flex items-center justify-center"
              ...
              {/* 3D Laptop Demo */}
              <div className="laptop-container relative mx-auto" style={{ 
                perspective: reduceMotion ? 'none' : '1500px', 
                maxWidth: '800px' 
              }}>
                {/* 3D Laptop with optimized animations */}
                <motion.div 
                  className="laptop"
                ...
                >
                  {/* Laptop Screen */}
                  <div className="laptop-screen website-shadow" style={{
                    width: '100%',
                    maxWidth: '700px',
                    height: 'auto',
                    minHeight: '400px',
                    ...
                    <div className="website-content" style={{
                      ...
                      overflow: 'hidden',
                      position: 'relative',
                      fontSize: '80%' // Reduced text size
                    }}>
                      {/* Website Header - Using bot's primary color */}
                      <div style={{ 
                      ...
                      {/* Website Hero Section - Darker overlay */}
                      <div style={{ 
                        padding: '20px 30px', 
                        ...
                        <div className="max-w-2xl">
                          ...
                          <div className="flex space-x-3">
                            <button className="px-3 py-1 text-white rounded-lg font-medium text-xs" style={{ backgroundColor: botConfig.primaryColor || '#4f46e5' }}>
                              Get Started
                            </button>
                      ...
                      <div className="p-5 flex-grow overflow-auto" style={{ 
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                          <div className="p-2 bg-white rounded-xl shadow-sm">
                            <div className="w-6 h-6 rounded-lg mb-2 flex items-center justify-center" style={{ backgroundColor: `${botConfig.primaryColor || '#4f46e5'}20` }}>
                              <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" style={{ color: botConfig.primaryColor || '#4f46e5' }}>
                        ...
                        {chatOpen && (
                          ...
                          >
                            ...
                            <div className="px-4 py-3 flex items-center justify-between text-white" style={{ 
                              background: `linear-gradient(135deg, ${botConfig.primaryColor || '#4f46e5'}, ${botConfig.secondaryColor || '#3730a3'})`,
                              ...
                              <div className="flex items-center">
                                <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center">
                                  {botConfig.iconStyle === 'message' ? (
                                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" style={{ stroke: botConfig.primaryColor || '#4f46e5' }}>
                            ...
                            <div className="bg-gray-50 px-3 py-2 border-b border-gray-200">
                              <input 
                                ...
                                style={{ accentColor: botConfig.primaryColor || '#4f46e5' }}
                              />
                            </div>
                                      ...
                                      >
                                        {msg.role === 'system' ? (
                                          ...
                                          <div
                                            ...
                                            style={{ 
                                              backgroundColor: msg.role === 'user' ? botConfig.primaryColor || '#4f46e5' : undefined,
                            ...
                            >
                              <input
                                ...
                                style={{ 
                                  outlineColor: botConfig.primaryColor || '#4f46e5',
                                  focusRing: botConfig.primaryColor || '#4f46e5',
                                }}
                              />
                              <button
                                type="submit"
                                ...
                                style={{ 
                                  backgroundColor: inputMessage.trim() ? (botConfig.primaryColor || '#4f46e5') : undefined,
                                }}
